...
//	@RequiresPermissions(value = {PermissionConstant.PERMSSION_RP_LIST})
	public void index() {
        ...
        String sql_m = "select r.id code, r.name value from eeda_role r where office_id=?";

        // 获取当前页的数据
        List<Record> roles = Db.find(sql_m, user.get("office_id"));
        setAttr("roles", roles);
		render("/eeda/profile/privilege/PrivilegeList.html");
	}
	//@RequiresPermissions(value = {PermissionConstant.PERMSSION_RP_LIST})
	//编辑和分配都是用这个list
	public void list() {
		...
		String code = null;
		if(rolename!=null){
			Role role = Role.dao.findFirst("select * from eeda_role where name=? and office_id=?",rolename,parentID);
			code = role.get("code");
		}
...
//	@RequiresPermissions(value = {PermissionConstant.PERMSSION_RP_CREATE})
	public void add(){
		render("/eeda/profile/privilege/RolePrivilege.html");
	}
...
//	@RequiresPermissions(value = {PermissionConstant.PERMSSION_RP_UPDATE})
	public void edit(){
		...
		render("/eeda/profile/privilege/RolePrivilegeEdit.html");
	}
}
