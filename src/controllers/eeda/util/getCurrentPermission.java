...
public class getCurrentPermission extends Controller{
	...
	public Map<String,String> currentHasPermission(Subject currentUser,ParentOfficeModel pom){
		String sql = "select distinct p.id, p.code, p.name,r.permission_code,r.is_authorize from eeda_permission p "
		        + "left join (select rp.* from eeda_user_role  ur left join eeda_role_permission  rp on rp.role_id = ur.role_id where ur.user_name ='" + currentUser.getPrincipal() + "' and  rp.office_id =  " + pom.getParentOfficeId() + ")r on r.permission_code = p.code ";
