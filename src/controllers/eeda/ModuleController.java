...
import controllers.eeda.profile.LoginUserController;
import controllers.eeda.util.DbUtils;
import controllers.eeda.util.ExpCalculator;
...
public class ModuleController extends Controller {
    ...
    public void getDataList(){
        ...
        String sql = "select id,"+ fieldName +" name from t_"+s.getLong("id");
//                +"where office_id="+LoginUserController.getLoginUser().get("office_id");
    ...
    public void addModule(){
    	...
    	module.set("office_id", LoginUserController.getLoginUser().get("office_id"));
    ...
    public void updateModuleSeq(){
    	...
    	//重新算序号
    	String sql = "select id, module_name, parent_id, office_id, seq from eeda_modules where office_id="
    			+LoginUserController.getLoginUser().get("office_id")+" order by seq";
    ...
    public void saveStructure() throws InstantiationException, IllegalAccessException{
        ...
        UserLogin user = LoginUserController.getLoginUser();
    ...
    private void eventHandle(Map<String, ?> dto, String module_id) {
        ...
        if(deleteIds.size()>0)
            Db.update("delete from eeda_module_event where module_id=? and id not in(?)", module_id, StringUtils.join(deleteIds, ","));
        ...
        for (Map<String, ?> row : event_list) {
            ...
            Record rec =Db.findFirst("select * from eeda_module_event where module_id=? and id=? ", module_id, id);
            if(rec!=null){//update
                String sql = "update eeda_module_event set event_name=?, event_type=?, event_script=? where id=?";
                int updateCount = Db.update(sql, event_name, event_type, event_script, id);
            }else{//insert
    ...
    private void authHandle(Map<String, ?> dto, String module_id) {
        List<Map<String, ?>> auth_list = (ArrayList<Map<String, ?>>)dto.get("auth_list");
        if(auth_list.size() ==0)
            return;
        ...
        if(roleIds.size()>0)
            Db.update("delete from eeda_module_permission where module_id=? and role_id not in(?)", module_id, StringUtils.join(roleIds, ","));
        
        for (Map<String, ?> row : auth_list) {
            
            List<Map<String, String>> role_auth= (List<Map<String, String>>)row.get("role_auth_list");
            if(row.get("role_id")==null && role_auth.size()==0)
                continue;
            String role_id= row.get("role_id").toString();
            logger.debug("role_id="+role_id);
            logger.debug(role_auth);
            for (Map<String, ?> role_auth_map : role_auth) {
                ...
                String is_auth = role_auth_map.get("bAuth")==Boolean.TRUE?"Y":"N";
                Record rec =Db.findFirst("select * from eeda_module_permission where module_id=? and role_id=? and permission_id=?", 
                        module_id, role_id, permission_id);
                if(rec!=null){//update
                    String sql = "update eeda_module_permission set permission_id=?, is_auth=?, office_id=? where id=?";
                    int updateCount = Db.update(sql, permission_id, is_auth, null, rec.get("id"));
                }else{//insert
                    String sql = "insert into eeda_module_permission (module_id, role_id, permission_id, is_auth, office_id) values(?, ?, ?, ?, ?)";
                    Db.update(sql, module_id, role_id, permission_id, is_auth, null);
                }
            } 
        }
    }
    ...
    private void activateModule(String module_id) {
        ...
        Db.update(" update eeda_modules set status = '启用' where id=?", module_id);
    ...
    public Record getOrderStructureDto(String module_id) {
        ...
        List<Record> structure_list = getStructureRecs(module_id);
    ...
    private List<Record> getActionList(String module_id) {
        ...
        if(user.isAdmin()){
            ...
            if(action_list.size() == 0){//如果为0，则取系统默认的几个按钮出来
                action_list = Db.find("select * from eeda_structure_action where module_id is null" );
            }
        }else{
            //获取user的权限,看看是否有对应权限
            String authSql="select  sa.*, mp.is_auth, mp.role_id, er.name role_name, ur.user_name "+
                    " from eeda_structure_action sa, eeda_module_permission mp, eeda_role er, eeda_user_role ur "+
            ...
            action_list = Db.find(authSql, module_id, UserLogin.getCurrentUser().getStr("user_name"));
    ...
    private List<Record> getEventList(String module_id) {
        List<Record> event_list = Db.find("select * from eeda_module_event where module_id=?", module_id);
        if (event_list == null) {
        ...
        for (Record record : event_list) {
            ...
            for (Map dto : commandList) {
                ...
                for (String fieldName : objs) {
                    ...
                    String transFieldName = "t_"+field.getLong("structure_id")+".f"+field.getLong("id")+"_"+field.getStr("field_name");
                    operation_obj_exp_key = operation_obj_exp_key.replaceAll(fieldName, transFieldName); 
                }
    ...
    private List<Record> buildValueRecList(Map dto) {
        ...
        for (Map expMap: setValueList) {
            ...
            Stack<String> paramStack = ExpCalculator.getParamsStack(exp_post);
            for (String paramFieldName : paramStack) {
                if(paramFieldName.indexOf(".")==-1)
                    continue;
                Record paramField = getFieldByName(paramFieldName);
                String transParamFieldName = "t_"+paramField.getLong("structure_id")+".f"+paramField.getLong("id")+"_"+paramField.getStr("field_name");
                exp_key = exp_key.replaceAll(paramFieldName, transParamFieldName);
            }
            expRec.set("exp", exp);
    ...
    private List<Record> getAuthList(String module_id) {
        List<Record> authRecs = Db.find("select * from eeda_module_permission where module_id=?", module_id);
        Map<Long, List<Record>> roleMap = new HashMap<Long, List<Record>>();
       
        for (Record record : authRecs) {
            List<Record> role_auth_list=null;
            Long role_id = record.getLong("role_id");
            if(roleMap.get(role_id) !=null){
                role_auth_list = roleMap.get(role_id) ;
                Record role_auth = new Record();
                role_auth.set("id", record.get("permission_id"));
                role_auth.set("bAuth", "Y".equals(record.get("is_auth"))?true:false);
                role_auth_list.add(role_auth);
            }else{
                role_auth_list = new ArrayList<Record>();
                roleMap.put(role_id, role_auth_list);
        ...
        List<Record> auth_list = new ArrayList<Record>();
        for (Entry<Long, List<Record>> entry: roleMap.entrySet()) {
            Long role_id = entry.getKey();
            List<Record> list = entry.getValue();
            Record rec = new Record();
            rec.set("role_id", role_id);
            rec.set("role_auth_list", list);
            auth_list.add(rec);
        }
        return auth_list;
    }

    private List<Record> getStructureRecs(String module_id) {
        List<Record> sRecs = Db.find("select * from eeda_structure where module_id=?", module_id);
        for (Record structure : sRecs) {
            String fieldSql = "select * from eeda_field where structure_id=? order by seq";
            ...
            if("弹出列表, 从其它数据表选取".equals(structure.get("ADD_BTN_TYPE"))){
                ...
                Record structureRec = Db.findFirst("select * from eeda_structure where id=?", structure_id);
                List<Record> fieldsRecs = Db.find("select * from eeda_field where structure_id=?", structure_id);
...
//        HomeController.loadMenu();
    ...
    public void getSysSetting(){
        ...
        List<Record> recs = Db.find("select * from eeda_setting where code =?", code);
        if(recs == null)
    ...
    public void getSysVar(){
        List<Record> recs = Db.find("select id, name from eeda_setting where code ='sys_var_default'");
        renderJson(recs);
    }
    ...
    public static Record getStructureByName(String name){
        return Db.findFirst("select id, name, module_id from eeda_structure where name =?", name);
    }
    /*
     * 传进来的参数应该是：运输单.单号
     */
    ...
    public static Record getFieldByName(String name){
        ...
        Record field = Db.findFirst("select * from eeda_field where structure_id=? and field_display_name =?", structure.get("id"),  field_name);
